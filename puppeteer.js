...
exports.generateHarAndScreenshot = async (url, proxy_server, username, password, request) => {
	...
	try {   
		...
		request.log(['HARANDSCREENSHOTINFO'],"EXTRA_HEADERS_WERE_SET_UP_AND_STTING_VIEWPORT", `${url} : ${pid} `)
		...
		if (AllowScreenshotRespCode.includes(response.status())) {
			...
			{
				request.log(['HARANDSCREENSHOTINFO'],"FULLPAGE_SCREENSHOT_TAKEN", `${url} : ${pid} : ${fullPageScreenshot} `)
			}
			...
			if (screenshot === 'Site Screenshot Timed Out') {
				...
				request.log(['HARANDSCREENSHOTINFO'],"SCREENSHOT_TAKEN", `${url} : ${pid} : ${screenshot} `)     
			}
			return {  
